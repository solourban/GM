(() => {
  const css = `
    .gm-map-empty-small{border:1px solid #e5dfd5;background:#fbfaf7;border-radius:14px;padding:18px!important;min-height:auto!important;display:block!important;color:#111827!important}
  ...
  `;
  if(!document.getElementById('step1UxPolishStyles')){const s=document.createElement('style');s.id='step1UxPolishStyles';s.textContent=css;document.head.appendChild(s)}
  ...
  function cardOf(el){let c=el;for(let i=0;c&&i<8;i++,c=c.parentElement){if(c.offsetWidth>360&&c.querySelectorAll('h1,h2,h3,h4,table').length)return c}return el&&el.parentElement}
  ...
  function mapEmpty(){const box=[...document.querySelectorAll('div,section')].find(el=>{const t=tx(el);return t.includes('지도 로딩 실패')&&t.includes('Kakao 지도 SDK 로드 실패')&&el.offsetHeight>140});if(!box||box.dataset.gmDone)return;box.dataset.gmDone='1';box.className=(box.className+' gm-map-empty-small').trim();box.innerHTML='<div class="t">지도 연동 필요</div><div class="d">Kakao API 키가 정상 연결되면 지도와 주변 입지 분석이 표시됩니다. 지금은 카카오맵 열기 버튼으로 주소를 확인하세요.</div>'}
  ...
  function run(){summaryTitle();mapEmpty();scheduleSmall();stakeholders()}
  document.addEventListener('DOMContentLoaded',run);setInterval(run,1200);run();
  window.GM?.patches?.register?.('step1-ux-polish',{version:'v1'});
})();
