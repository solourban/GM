(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .v2-card, .v2-result-card { background:#fff; color:var(--ink); border:1px solid var(--line); border-radius:22px; padding:22px; box-shadow:0 18px 48px rgba(0,0,0,.08); }
      .v2-card { min-height:174px; }
      ...
      .v2-btn, .v2-secondary-btn, .v2-small-btn, .v2-danger-btn { min-height:42px; border-radius:11px; padding:0 16px; font-weight:900; cursor:pointer; border:1px solid var(--line); }
      ...
      @media (max-width:900px){ .site-header{padding:8px 0;} .header-inner{align-items:flex-start; flex-direction:column; gap:12px; min-height:0; padding:0 18px;} .brand-mark{width:34px;height:34px;border-radius:9px;font-size:17px}.brand-text h1{font-size:17px}.brand-text p{font-size:10.5px}.v2-tabs{justify-content:flex-start;} .hero{min-height:560px;} .v2-form{grid-template-columns:1fr 110px}.v2-court,.v2-case,.v2-btn{grid-column:1/-1}.v2-btn{width:100%;} }
      @media (max-width:720px){ .container{padding-left:18px;padding-right:18px}.site-header{padding:6px 0}.header-inner{gap:9px;padding:0 14px}.v2-tabs{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));width:100%;gap:7px}.v2-tab{grid-column:span 2;width:100%;padding:8px 5px;font-size:12px;text-align:center}.v2-tab:nth-child(n+4){grid-column:span 3}.hero{min-height:auto; padding:24px 0 28px;} .hero-copy{text-align:left;margin-bottom:18px;} #v2HomePanels{min-height:0}.v2-card{min-height:0;padding:18px;border-radius:18px}.v2-card h3,.v2-result-card h3{font-size:19px}.v2-form{grid-template-columns:1fr;gap:10px;margin-top:14px}.v2-field input,.v2-field select{padding:12px 13px;font-size:14px}.v2-cta-row .v2-btn,.v2-cta-row .v2-secondary-btn{width:100%;}.v2-tab-results-section{padding-top:20px;scroll-margin-top:130px}.v2-date-card-list{display:grid;gap:10px;margin-top:12px}.v2-date-table-wrap{display:none}.v2-result-card{border-radius:16px;padding:18px} }
    `;
  ...
  function renderNextStep() {
    ...
    return `<section class="v2-result-card"><h3>다음 단계</h3><p class="v2-note">매각물건명세서와 등기부 기준으로 최선순위·임차인·특수권리를 입력하면 권리분석 준비가 됩니다.</p><div class="v2-grid compact">${info('최선순위', hasMalso ? '입력 중' : '미입력')}${info('임차인 입력', `${state.manual.tenants.length}명`)}${info('특수권리 입력', `${state.manual.specials.length}건`)}</div><div class="v2-cta-row"><button class="v2-btn" data-action="open-step2">${state.step2Visible ? 'Step 2 입력 계속하기' : 'Step 2 명세서 입력 시작'}</button>${state.step2Visible ? '<button class="v2-secondary-btn" data-action="close-step2">Step 2 접기</button>' : ''}</div></section>`;
  }
  ...
  function canAnalyze() {
    return hasValue(state.manual.malso, ['date','type','holder','amount']) || state.manual.tenants.some((t) => hasValue(t, ['name','moveIn','fixed','deposit'])) || state.manual.specials.some((s) => hasValue(s, ['type','holder','date','amount']));
  }
  ...
  function renderAnalysis() {
    ...
    return `<section class="v2-result-card analysis" id="analysisCard"><span class="v2-risk-badge ${cls}">위험도 ${label}</span><h3>권리분석 결과</h3><p class="v2-note">${esc(state.reportAt)} 기준 실행 결과입니다.</p><div class="v2-grid compact">${info('위험 등급', label)}${info('인수금액', won(inherited))}${info('임차인', `${tenants.length}명`)}${info('권리', `${rights.length}건`)}</div><ul class="v2-analysis-list">${items.map((x) => `<li>${esc(x)}</li>`).join('')}</ul>${renderTenantDetails(tenants)}${renderRightDetails(rights)}<p class="v2-note">이 결과는 입력값 기반 1차 판단입니다. 실제 입찰 전 등기부, 매각물건명세서, 전입세대열람, 현장조사를 다시 확인해야 합니다.</p></section>`;
  }
