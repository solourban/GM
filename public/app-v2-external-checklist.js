(() => {
  ...
  const ITEMS = [
    {
      ...
      title: '지역 분위기·단지 평판',
      source: '호갱노노 등',
      guide: '소음, 악취, 언덕, 주차, 하자, 동·라인 이슈가 반복 언급되는지 확인',
    },
    {
      id: 'supply',
      title: '입주물량·공급 부담',
      source: '아실 등',
      guide: '향후 입주물량, 주변 신축 공급, 전세·매매 공급 부담 확인',
    },
    {
      id: 'management',
      title: '관리비·주차·공용관리',
      source: 'K-apt 등',
      guide: '세대당 주차대수, 공용관리비, 장기수선충당금, 관리비 과다 여부 확인',
    },
    {
      id: 'finance',
      title: '금융권 시세·대출 기준',
      source: 'KB부동산 등',
      guide: 'KB시세 존재 여부, 일반평균가·하위평균가, 현금+대출 가능 범위 확인',
    },
    {
      id: 'listing',
      title: '현재 매물·호가·등록기간',
      source: '네이버 부동산 등',
      guide: '같은 단지·평형 현재 호가, 최초 등록일, 가격 변경 여부 확인',
    },
  ];
  ...
  function loadChecklist(currentReport = report()) {
    try {
      ...
      return { ...defaultState(), ...(parsed || {}), checks: { ...defaultState().checks, ...(parsed?.checks || {}) }, memos: { ...defaultState().memos, ...(parsed?.memos || {}) } };
    } catch (_) {
      return defaultState();
    }
  }
  ...
  function injectFinalJudgmentStatus() {
  ...
  function updateFromDom() {
    ...
    document.querySelectorAll('[data-external-check]').forEach((input) => {
      data.checks[input.dataset.externalCheck] = Boolean(input.checked);
    });
    document.querySelectorAll('[data-external-memo]').forEach((input) => {
