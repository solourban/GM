(() => {
  ...
  function qualitySnapshot() {
    ...
    const checks = [
      { key: 'basic', label: '기본정보', ok: hasBasicInfo(state), miss: '사건 기본정보 없음' },
      { key: 'rights', label: '권리분석', ok: hasRightsAnalysis(report), miss: '권리분석 결과 없음' },
      { key: 'minBid', label: '최저가', ok: minBid > 0, miss: '최저매각가격 없음' },
      { key: 'location', label: '입지 좌표', ok: Boolean(location?.x && location?.y), miss: '주소 좌표 미확인' },
      { key: 'bCode', label: '법정동코드', ok: Boolean(location?.bCode), miss: '법정동코드 미확인' },
      { key: 'trades', label: '실거래가', ok: tradeCount > 0, miss: '실거래가 표본 없음' },
      { key: 'comparison', label: '가격비교', ok: Boolean(trades?.comparison?.avgRatio), miss: '최저가와 실거래가 비교 불가' },
      { key: 'final', label: '종합판단', ok: Boolean(finalJudgment?.decision?.label), miss: '입찰 판단 종합 미생성' },
    ];
    const pass = checks.filter((item) => item.ok).length;
    const total = checks.length;
    const ratio = total ? Math.round((pass / total) * 100) : 0;
    let label = '낮음';
    let tone = 'danger';
    ...
    if (ratio >= 75) {
      ...
      label = '보통';
      tone = 'warn';
      message = '핵심 데이터 일부가 부족합니다. 누락 항목을 확인한 뒤 판단해야 합니다.';
    }
    return { checks, pass, total, ratio, label, tone, message, missing: checks.filter((item) => !item.ok).map((item) => item.miss), tradeCount };
  }
    ...
    return `
      <section class="v2-card" id="${CARD_ID}">
        ...
        <div class="v2-grid compact">
          ...
          <div class="v2-info wide"><div class="k">누락 항목</div><div class="v">${esc(missingText)}</div></div>
