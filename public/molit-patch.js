(() => {
  ...
  function krwFromMan(value) { const man = parseAmount(value); if (!man) return '-'; const eok = Math.floor(man / 10000); const rest = man % 10000; const parts = []; if (eok) parts.push(`${eok}억`); if (rest) parts.push(`${rest.toLocaleString('ko-KR')}만`); return parts.join(' ') + '원'; }
  function getAddress(raw) { return raw?.basic?.['소재지'] || raw?.basic?.['주소'] || ''; }
  function normalizeAddress(value) { return String(value || '').replace(/서울특별시|서울시/g, '서울').replace(/부산광역시|부산시/g, '부산').replace(/대구광역시|대구시/g, '대구').replace(/인천광역시|인천시/g, '인천').replace(/광주광역시|광주시/g, '광주').replace(/대전광역시|대전시/g, '대전').replace(/울산광역시|울산시/g, '울산').replace(/경기도/g, '경기').replace(/세종특별자치시/g, '세종').replace(/\s+/g, ' ').trim(); }
  function guessLawdCd(raw) { const address = normalizeAddress(getAddress(raw)); const sorted = [...LAWD_CODES].sort((a, b) => b[1].length - a[1].length); for (const [sido, sigungu, code] of sorted) { if (address.includes(sido) && address.includes(sigungu)) return { code, label: `${sido} ${sigungu}` }; } return { code: '', label: '' }; }
  function cleanBuildingName(value) { return String(value || '').replace(/\s+/g, '').replace(/제?\d+동|제?\d+호|\d+층|\d+호/g, '').replace(/[()\[\]{}·,._\-]/g, '').trim(); }
  function guessBuildingName(raw) { const address = getAddress(raw); const parens = [...String(address).matchAll(/\(([^()]+)\)/g)].map((m) => m[1]); for (const p of parens.reverse()) { const parts = p.split(',').map((x) => x.trim()).filter(Boolean); const candidate = cleanBuildingName(parts[parts.length - 1]); if (candidate && !/^[가-힣]+동$/.test(candidate)) return candidate; } return ''; }
  function matchHint(name) { const n = cleanBuildingName(name); if (!n) return { quality: 'none', text: '건물명 필터 없음: 지역 전체 거래입니다.' }; if (n.length < 4 || ['강남', '역삼', '자곡', '논현', '서초', '송파'].includes(n)) return { quality: 'weak', text: '필터가 너무 넓습니다. 같은 건물 시세가 아니라 주변/유사명 거래일 수 있습니다.' }; return { quality: 'strong', text: '건물명 기준으로 비교적 강하게 필터링합니다.' }; }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .molit-actions button[disabled] { opacity:.45; cursor:not-allowed; }
  ...
  function findLocationCard() { return [...document.querySelectorAll('.step1-extra-card h4')].find((h) => h.textContent.includes('입지분석'))?.closest('.step1-extra-card'); }
    ...
    card.insertAdjacentHTML('beforeend', `
      <div class="molit-card">
        ...
        <div class="molit-form">
          <label>거래 유형 <select id="molitTradeType"><option value="auto">자동: 아파트+오피스텔+연립다세대</option><option value="apt">아파트</option><option value="offi">오피스텔</option><option value="rh">연립다세대</option></select></label>
        ...
        <div class="molit-actions">
          <button id="molitLookupBtn" onclick="lookupMolitTrades()">실거래가 조회</button>
  ...
  async function checkMolitConfig() {
    ...
    const btn = document.getElementById('molitLookupBtn');
    const result = document.getElementById('molitResult');
    if (!state) return false;
    try {
      const data = await fetch('/api/config', { cache: 'no-store' }).then((r) => r.json());
      window.__hasMolitKey = Boolean(data.hasMolit);
      state.textContent = data.hasMolit ? '설정됨' : 'MOLIT_API_KEY 필요';
      if (btn) btn.disabled = !data.hasMolit;
      if (!data.hasMolit && result) {
        result.innerHTML = '<div class="note danger-note">실거래가 조회를 사용하려면 Railway Variables에 MOLIT_API_KEY 또는 DATA_GO_KR_KEY를 먼저 등록해야 합니다. 등록 후 재배포하고 진단 버튼에서 확인하세요.</div>';
      }
      return Boolean(data.hasMolit);
    } catch {
  ...
  window.lookupMolitTrades = async function() {
    const result = document.getElementById('molitResult');
    if (!result) return;
    if (!window.__hasMolitKey) {
      const ok = await checkMolitConfig();
      if (!ok) return;
    }
    ...
    try {
      ...
      if (!trades.length) return result.innerHTML = `<div class="note warn-note">조회된 거래가 없습니다. 현재 조건의 전체 원천 거래수는 ${data.rawCount || 0}건입니다. 건물명 필터를 줄이거나 계약월을 바꿔보세요.<br>${esc(typeText)}</div>`;
      ...
      const matchWarning = data.matchQuality === 'weak' ? '<div class="molit-match-warning" data-molit-match="weak">건물명 필터가 약합니다. 이 결과는 같은 건물 시세가 아니라 주변/유사명 거래일 수 있으므로 시세 3단 시나리오 자동 반영은 권장하지 않습니다.</div>' : data.matchQuality === 'strong' ? '<div class="molit-match-ok" data-molit-match="strong">건물명 필터가 적용된 결과입니다. 그래도 전용면적·층·도로명을 함께 확인하세요.</div>' : '<div class="molit-match-warning" data-molit-match="none">건물명 필터 없이 지역 전체 거래를 조회했습니다. 참고용 주변 시세입니다.</div>';
      result.innerHTML = `
        ${matchWarning}
        ...
        <div class="molit-summary"><div class="box"><div class="k">거래 수</div><div class="v">${trades.length}건</div></div><div class="box"><div class="k">평균 거래금액</div><div class="v">${krwFromMan(avg)}</div></div><div class="box"><div class="k">최저 거래금액</div><div class="v">${krwFromMan(min)}</div></div><div class="box"><div class="k">최고 거래금액</div><div class="v">${krwFromMan(max)}</div></div></div>
        <div class="molit-table-wrap"><table class="molit-table"><thead><tr><th>구분</th><th>계약일</th><th>건물명</th><th>전용㎡</th><th>층</th><th>거래금액</th><th>동/도로명</th></tr></thead><tbody>${trades.slice(0, 40).map((t) => `<tr><td>${esc(t.tradeTypeLabel)}</td><td>${esc(t.dealYear)}.${esc(t.dealMonth)}.${esc(t.dealDay)}</td><td>${esc(t.aptName)}</td><td>${esc(t.area)}</td><td>${esc(t.floor)}</td><td><b>${esc(t.dealAmount)}만원</b>${t.cancelDate ? '<br><span class="watch-small">해제 가능</span>' : ''}</td><td>${esc(t.dong)} / ${esc(t.roadName)}</td></tr>`).join('')}</tbody></table></div>
      `;
    } catch (e) {
  ...
  const wait = setInterval(() => {
    if (typeof window.renderStep1 === 'function') {
      ...
      window.renderStep1 = function patchedMolitRenderStep1(raw, elapsed) { original(raw, elapsed); setTimeout(() => enhanceMolitCard(raw), 0); };
    }
  }, 50);
})();
