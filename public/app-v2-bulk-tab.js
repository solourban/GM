(() => {
        ...
        <table class="v2-detail-table">
          ...
          <tbody>
            ${state.rows.map((row) => {
              ...
              const caseNo = clean(row.raw?.caseNo || basic['사건번호'] || `${row.year}타경${row.serial}`);
              return `
                <tr>
                  ...
                  <td>${row.ok ? `<button type="button" class="v2-small-btn" data-bulk-action="open" data-court="${esc(row.court)}" data-year="${esc(row.year)}" data-serial="${esc(row.serial)}">단건조회로 열기</button>` : '-'}</td>
                </tr>
              `;
            }).join('')}
          </tbody>
  ...
  function renderResults() {
    ...
    return `
      <section class="v2-card" id="${RESULT_ID}">
        ...
        <h3>여러 사건 조회 결과</h3>
        <p class="v2-note">성공한 사건은 단건조회로 열어 권리분석·입지·실거래가 검토로 이어갈 수 있습니다.</p>
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('[data-bulk-action="open"]').forEach((button) => {
      if (button.dataset.bound) return;
      button.dataset.bound = '1';
      button.addEventListener('click', () => openSingle(button));
    });
  }
