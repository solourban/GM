  ...
  rs.innerHTML = `
    ...
    <div class="subcard">
      ...
      <table class="basic-table"><tbody>
        ${Object.entries(basic).filter(([k,v]) => v).map(([k, v]) => `<tr><th>${escapeHtml(k)}</th><td>${escapeHtml(v)}</td></tr>`).join('')}
      </tbody></table>
    </div>
    ...
    <div class="step2-divider">
      ...
      <p class="muted">
        권리분석을 위해 매각물건명세서에 표시된 정보가 필요합니다.<br>
        대법원 경매정보 페이지에서 <b>매각물건명세서</b>를 열어서 보시면 다음 값이 있어요.
      </p>
    </div>
    ...
    <div class="subcard input-card">
      ...
      ${tenantCandidates.length > 0 ? `
        <p class="muted" style="margin-top:10px">
          <b>참고:</b> 자동 조회된 임차인 — ${tenantCandidates.map(t => t.name).join(', ')}
        </p>
      ` : ''}
    </div>
...
window.addRight = function() {
  ...
  div.innerHTML = `
    <label>접수일 <input type="date" data-k="date"></label>
    <label>종류
      <select data-k="type">
        <option>근저당</option>
        <option>저당</option>
    ...
    <label>권리자 <input type="text" data-k="holder"></label>
    <label>금액 <input type="number" data-k="amount" placeholder="원"></label>
...
window.addTenant = function(prefillName = '') {
  ...
  div.className = 'input-row';
  div.innerHTML = `
    <label>이름 <input type="text" data-k="name" value="${escapeHtml(prefillName)}"></label>
    <label>전입신고일 <input type="date" data-k="moveIn"></label>
    <label>확정일자 <input type="date" data-k="fixed"></label>
    <label>보증금 <input type="number" data-k="deposit" placeholder="원"></label>
    <button type="button" class="btn-remove" onclick="this.parentElement.remove()">×</button>
  `;
  list.appendChild(div);
};
...
window.addSpecial = function() {
  ...
  div.innerHTML = `
    ...
    <label>접수일 <input type="date" data-k="date"></label>
    <label>권리자 <input type="text" data-k="holder" placeholder="있으면"></label>
    <label>금액 <input type="number" data-k="amount" placeholder="원 (없으면 0)"></label>
    <button type="button" class="btn-remove" onclick="this.parentElement.remove()">×</button>
  `;
  list.appendChild(div);
};
...
function renderReport(report) {
  ...
  rs.innerHTML = `
    <div style="text-align:right; margin-bottom:16px">
      <button class="btn-back" onclick="location.reload()">← 새 사건 분석</button>
    </div>
    ...
    <div class="subcard">
      ...
      ${report.rights.length === 0 ? '<p class="muted">등록된 권리가 없습니다</p>' : `
        <table class="rights-table">
          ...
          <tbody>
            ${report.rights.map(r => `
              <tr class="${r.isMalso ? 'malso-row' : ''}">
                ...
                <td>${escapeHtml(r.type)}${r.isMalso ? '<span class="tag malso"> 말소기준</span>' : ''}</td>
                <td>${escapeHtml(r.holder)}</td>
                <td style="text-align:right">${formatMoney(r.amount)}</td>
                <td style="text-align:center"><span class="tag ${r.status === '인수' ? 'inherit' : 'extinct'}">${escapeHtml(r.status)}</span></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
    ...
    <div class="subcard">
      ...
      ${report.risk.flags.map(f => `
        <div class="note ${f.sev === 'danger' ? 'danger-note' : f.sev === 'warn' ? 'warn-note' : ''}">
          ${f.sev === 'danger' ? '🚨' : f.sev === 'warn' ? '⚠️' : '✓'} ${escapeHtml(f.msg)}
        </div>
      `).join('')}
    </div>
    ...
    <div class="subcard">
      ...
      <table class="rights-table">
        ...
        <tbody>
          ${report.baedang.allocations.map(a => `<tr><td>${a.order}순위</td><td>${escapeHtml(a.label)}</td><td style="text-align:right">${formatMoney(a.amount)}</td></tr>`).join('')}
