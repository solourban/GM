...
- `innerHTML` 사용이 많은 구조라 `innerhtml-escape-guard`로 core/date/saved/copy/final/validate/allocation/risk/onbid/bid-plan/location/spec/external/bulk/essential/service의 핵심 escape 계약을 고정했다.
- 남은 loaded v2 파일 중 date-source/candidate-stack/molit/confidence/case-sync 계열의 DOM helper와 escape 범위를 이어서 정적 guard에 편입한다.
