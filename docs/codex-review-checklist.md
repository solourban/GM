...
| `public/app-v2-molit-trades.js` | 국토부 실거래가 카드 | `v2MolitTradeCard` | `v2LocationCard` | `auction-note:v2:location-geocode`, `auction-note:v2:molit-trades` | `/api/molit/trades` | 아니오 | render helper escape 사용 | `external-api-proxy`, `api-contract-hardening` | 중 | 서버 MOLIT fetch timeout 적용 |
...
- `innerHTML` 사용이 많은 구조라 `innerhtml-escape-guard`로 validate/allocation/risk/onbid/bid-plan/location/spec/external/bulk/essential/service의 핵심 escape 계약을 고정했다.
- 남은 loaded v2 파일 중 core/date/saved/copy/final 계열은 DOM helper와 escape 범위를 이어서 정적 guard에 편입한다.
