...
| `src/dateRecommendations.js` | 매각기일 추천 후보 수집/검증 | 해당 없음 | 해당 없음 | 해당 없음 | 외부 법원 API | 아니오 | clean 함수 사용 | `date-recommendations-regression` | 중 | `debug`가 API 응답에 포함됨 |
...
| `public/app-v2-core.js` | 홈 탭, 검색, Step1/Step2/권리분석 기본 렌더 | `v2HomePanels`, `v2TabResultsSection`, `step2InputCard`, `analysisCard` | `resultsSection`, `.hero-inner`, `.header-inner` | 없음 | `/api/courts`, `/api/fetch`, `/api/analyze` | 예 | `esc`, `textContent` 혼합 | `public-scripts`, `tab-results-layout`, `case-scope-regression` | 높음 | 화면 구조의 중심. 홈 화면 여백/hero도 여기서 제어 |
| `public/app-v2-analyze-bridge.js` | `/api/analyze` 요청/응답 bridge | 없음 | `window.fetch` | 없음 | `/api/analyze` | 아니오 | 해당 없음 | `analyzer`, `public-scripts` | 높음 | 서버 `result`를 프론트 `report`로 보정 |
...
| `public/app-v2-validate.js` | Step2/분석 경고 유지 | `v2Step2GuardMessage`, `v2Analyze*Message` | `step2InputCard`, `analysisCard` | 없음 | 없음 | 예 | 정적/내부 문자열 위주 | `case-reset-safety` | 중 | 분석 실패 시 입력 유지 UX |
| `public/app-v2-allocation.js` | 배당/인수 설명 보강 | `allocationExplainCard` | `analysisCard` | 없음 | 없음 | 예 | `esc` 확인 필요 | 없음 | 중 | 분석 카드 후속 보강 |
...
| `public/app-v2-risk-brief.js` | 위험 요약 카드 | `v2RiskBriefCard` | `v2BiddingSummaryCard` | 없음 | 없음 | 예 | `esc` 확인 필요 | `external-checklist-regression` | 중 | 최종 판단 카드들과 순서 의존 |
| `public/app-v2-copy-summary.js` | 최종 복사용 요약 | `v2CopySummaryCard`, `v2CopySummaryBtn` | bid/funding/checklist cards | `auction-note:v2.2:bid-plan:`, 후보/위치/실거래 storage | 없음 | 예 | `esc`, `textContent` 혼합 | `bid-plan-calculation` | 중 | 저장 후보/입찰가 snapshot 연결 |
| `public/app-v2-bid-plan.js` | 입찰가·자금 계산 카드 | `v2BidPlanCard`, `v2PlannedBidInput`, `v2BidPlan_*` | `v2PreBidChecklistCard`, `v2FundingReviewCard`, `v2BidRangeCard` | `auction-note:v2.2:bid-plan:` | 없음 | 예 | `esc`, `textContent` 사용 | `bid-plan-calculation` | 높음 | 계산식 변경 시 테스트 갱신 필요 |
...
| `public/app-v2-date.js` | 매각기일 추천 탭/결과 | `v2DateResultCard`, `v2DateEmptyStateCard`, `v2DateSortControls` | `v2TabResultsSection`, 검색 input | 없음 | `/api/recommendations/by-date` | 예 | `esc` 사용 | `date-recommendations-regression`, `tab-results-layout` | 높음 | 지시서의 `/api/date/recommendations`와 실제 경로가 다름 |
| `public/app-v2-date-source.js` | 날짜 후보를 검색 탭으로 handoff | `v2DateSourceCard`, memo controls | `resultsSection`, search panel | `auction-note:v2:selected-date-candidate`, `auction-note:v2:date-candidate-memo:` | 없음 | 아니오 | `textContent` 사용 | `case-scope-regression` | 중 | 후보 메모 저장 |
| `public/app-v2-candidate-stack.js` | 임시/저장 후보 stack | `v2CandidateStackCard`, `v2SavedTopFiveCard` | `v2TabResultsSection`, search input | `auction-note:v2:date-candidate-stack`, `auction-note:v2:saved-candidates`, memo key | 없음 | 아니오 | DOM API 중심 | `case-scope-regression` | 중 | localStorage 저장 후보 |
| `public/app-v2-saved-tab.js` | 저장 후보 TOP 5 탭 | `v2SavedTabControlsCard`, `v2SavedTabRuntimeCard`, `v2SavedMobileCards` | `v2TabResultsSection` | `auction-note:v2:saved-candidates`, memo key | 없음 | 예 | `esc` 사용 | `tab-results-layout` | 중 | 저장 후보 목록/검색 handoff |
...
| `public/app-v2-location.js` | 위치/카카오 지도/좌표 | `v2LocationCard` | `resultsSection` | `auction-note:v2:location-geocode` | `/api/location/geocode`, `/api/config`, `/api/kakao/maps-sdk.js` | 예 | `esc` 사용, 일부 `next.innerHTML` 교체 | `location-map-card`, `server-security` | 높음 | Kakao JS key는 서버 프록시 경유 |
| `public/app-v2-molit-trades.js` | 국토부 실거래가 카드 | `v2MolitTradeCard` | `v2LocationCard` | `auction-note:v2:location-geocode`, `auction-note:v2:molit-trades` | `/api/molit/trades` | 아니오 | render helper escape 확인 필요 | `external-api-proxy` | 중 | 서버 MOLIT fetch timeout 미적용 |
| `public/app-v2-final-judgment.js` | 최종 판단 카드 | `v2FinalJudgmentCard` | 위치/실거래/입찰가/checklist cards | `auction-note:v2:final-judgment`, 위치/실거래 keys | 없음 | 아니오 | DOM helper 중심 | `bid-plan-calculation` | 중 | bid-plan snapshot 반영 |
| `public/app-v2-external-checklist.js` | 외부 확인 체크리스트 | `v2ExternalVerification*` | `resultsSection`, risk/location/trade/final cards | `auction-note:v2:external-verification:` | 없음 | 예 | `esc`, `textContent` 혼합 | `external-checklist-regression` | 중 | 사건별 localStorage 메모 |
| `public/app-v2-confidence.js` | 판단 신뢰도 카드 | `v2DecisionConfidenceCard` | final/location/trade cards | final/location/trade keys | 없음 | 아니오 | DOM helper 중심 | `result-order-regression` | 중 | final 카드와 순서 의존 |
| `public/app-v2-case-sync-status.js` | 사건 동기화 상태 | `v2CaseSyncStatusCard` | final/molit/confidence cards | case/final/location/trade/active keys | 없음 | 아니오 | DOM helper 중심 | `case-scope-regression` | 중 | 사건 변경 시 stale data 감지 |
| `public/app-v2-final-copy-bridge.js` | 최종 복사 bridge | final copy card ids | copy/final/molit cards | final/location/trade keys | 없음 | 아니오 | `textContent` 사용 | `result-order-regression` | 낮음 | 복사용 텍스트 생성 |
...
- `/api/analyze`는 브라우저에서는 bridge가 `report`를 보정하지만, 서버 원응답은 `result`만 내려준다.
- `/api/recommendations/by-date`는 `debug`를 응답에 포함한다. 프론트는 사용하지 않지만 외부 노출 관점에서 축소 여부 검토가 필요하다.
- `/api/molit/trades`의 유형별 내부 catch는 `e.message`를 `tradeTypes.error`로 담을 수 있다.
- `innerHTML` 사용이 많은 구조라 테스트가 있는 파일부터 escape 함수 사용 범위를 계속 좁혀 확인해야 한다.
